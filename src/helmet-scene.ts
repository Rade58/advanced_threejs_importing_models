...
// helmet is a bit more complicated and better looking than
// previous duck model

// it contains a lot of textures, so when you load it
// there is not much place for cherry picking the things you want
// and the thing you don't want,

// in this case, we need to add entire scene to our scene
...
if (canvas) {
  ...
  gltfLoader.load(
    ...
    (gltf) => {
      console.log({ gltf });
      // so we are not going to search for the mesh and do something like this
      // scene.add(gltf.scene.children[0]);
      // because this will add only one part of the helmet, and that can be what every is first child
      // it can be some perspective camera, it can be som pipe, some glasses
      // but not the entire thing

      // so we do this
      // but we won't be doing it like this
      // scene.add(gltf.scene); // this would still work

      // let's try looping and adding

      /* for (const element of gltf.scene.children) {
        scene.add(element); // but this is lso wrong
        // you wil see, you don't see entire model, only some meshes are there
        // this is because you are moving stuff from a scene of the model to our scene
        // during look, which means you will mess with indexes
        // 

      }
     */
      // so it is better to use while loop
      // where you always check for length
      // loop will stop when length reaches 0
      //

      // 0 is falsy as you know
      /* while (gltf.scene.children.length) {
        scene.add(gltf.scene.children[0]); // we alsways move first one and put it to our scene

        // we see now entire model
        // looks great. Maybe not so much good as in preview image, but it looks ok
        // latter we will learn how we can make it more relaistic looking
      }
      */
      // but since people aare afraid of using while loop because
      // they are afraid of stacj overflow
      // people tend to create new array
      // and use it in for of loop instead of children array

      // we are copying things by doing this spread
